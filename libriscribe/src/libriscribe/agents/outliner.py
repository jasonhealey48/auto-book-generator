...
class OutlinerAgent(Agent):
    ...
    def execute(self, project_knowledge_base: ProjectKnowledgeBase, output_path: Optional[str] = None) -> None:
        ...
        try:
            ...
            initial_prompt = voice_lead(project_knowledge_base, initial_prompt)
    ...
    def generate_scene_outline(self, project_knowledge_base: ProjectKnowledgeBase, chapter: Chapter):
        ...
        try:
            ...
            * Emotional Beat: (The primary emotion conveyed in the scene)
            ...
            return False
    def _panels_to_scenes(self, chapter: Chapter) -> None:
        """Graphic Novel: turn each '- Panel N: <visual>' line in the
        chapter summary into a Scene (panel). No prose scene generation.
        """
        chapter.scenes = []
        summary = (chapter.summary or "").replace('\r\n', '\n').replace('\r', '\n')
        for line in summary.split('\n'):
            s = line.strip()
            low = s.lower()
            if not low.startswith('- panel') and not low.startswith('panel'):
                continue
            body = s
            if low.startswith('- '):
                body = s[2:].strip()
        ...
        chapter.scenes.sort(key=lambda sc: sc.scene_number)
