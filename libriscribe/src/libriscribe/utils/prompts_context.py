...
# ---------------------------------------------------------------------------
# Graphic Novel mode: the story is carried by panels (images), not prose.
# The writer outputs a tight PANEL SCRIPT (visual + dialogue + narration);
# almost no narrative prose is written.
# ---------------------------------------------------------------------------

GRAPHIC_OUTLINE_PROMPT = """
Create a PANEL-BASED outline for a {genre} GRAPHIC NOVEL titled "{title}" (categorized as {category}).
The book is written in {language}.
...
Format EXACTLY (Markdown, repeat per chapter):
# Chapter1: [Chapter Title]
### Summary
- Panel1: [One-line visual description of what the panel shows]
- Panel2: [One-line visual description]
- Panel3: [One-line visual description]
...
Rules:
- Each "- Panel N:" line is ONE short visual sentence (the artist draws it). Aim for 3-6 panels per chapter.
- Do NOT write prose, narration, or dialogue here — only what is SEEN.
- If there's a book length: ({book_length}), adjust the number of chapters accordingly.
- State the total number of chapters definitively. No optional chapters.
Return the panel outline in this exact Markdown structure.
IMPORTANT: The content should be written entirely in {language}.
"""

GRAPHIC_SCENE_PROMPT = """
Write the PANEL SCRIPT for Panel {scene_number} of {total_scenes} in Chapter {chapter_number}: {chapter_title}
of the {genre} GRAPHIC NOVEL "{book_title}" (written in {language}).

Chapter Summary:
{chapter_summary}

This panel's VISUAL (what the artist should draw):
{scene_summary}

Characters in this panel: {characters}
Setting: {setting}
Goal of the panel: {goal}
Emotional beat: {emotional_beat}

Output a STRICT panel script. Almost no prose — the image carries the story.
Format EXACTLY:

[Panel {scene_number}]
VISUAL: <one or two sentences describing what is drawn — composition, characters' poses, mood, lighting>
NARRATION: <one short narrator caption, max 12 words, spoken by the narrator>
DIALOGUE:
  <Character Name>: <the line this character says>
  <Character Name>: <another line, if needed>

Rules:
- VISUAL is for the illustrator only (it becomes the image prompt). Keep it vivid but brief.
- NARRATION is a tiny caption; keep it under 12 words.
- DIALOGUE lines are what characters SAY, one per character. Keep each line short (under 15 words).
- Do NOT add exposition, description, or prose outside these three blocks.
- Use language and tone appropriate for the genre and audience.
IMPORTANT: The content should be written entirely in {language}.
"""

GRAPHIC_EDITOR_PROMPT = """
You are a graphic-novel editor polishing a PANEL SCRIPT for voice consistency.
Target author voice: {author_name}. Target tone: {tone}.

Current panel script:
---
{panel_script}
---

Tasks:
- Tighten each DIALOGUE line so it sounds like {author_name}'s voice (rhythm, attitude, wit) while staying age-appropriate.
- Make the NARRATION line match the narrator's voice and the panel's mood.
- Keep VISUAL unchanged (it drives the artwork).
- Do NOT add prose. Return the FULL revised script in the SAME [Panel N] / VISUAL / NARRATION / DIALOGUE structure.
IMPORTANT: The content should be written entirely in {language}.
"""
